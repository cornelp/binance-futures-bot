...
class ScalpingStrategy extends AbstractStrategy {
  run() {
    // get sma 5
    const line5 = indicators.sma(this.getCandleData().mapIntoField("close"), 5);
    // get sma 8
    const line8 = indicators.sma(this.getCandleData().mapIntoField("close"), 8);
    // get sma 13
    ...
    this.crossOver =
      indicators.crossOver(line5, line8) && indicators.crossOver(line5, line13);

    this.crossUnder =
      indicators.crossUnder(line5, line8) &&
      indicators.crossUnder(line5, line13);

    console.log(this.crossOver, this.crossUnder);
  }
  ...
  canClosePosition() {
    ...
    this.isCurrentSide("SELL") ? this.isSignalLong() : this.isSignalShort();
  }
}
